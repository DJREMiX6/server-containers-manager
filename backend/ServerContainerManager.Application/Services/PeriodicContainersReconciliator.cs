...
namespace ServerContainerManager.Application.Services
{
        ...
        IOptions<DockerContainersReconciliationOptions> reconciliationOptions) : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            _logger.LogInformation("Set Periodic Timer delay to {PeriodicDelayMilliseconds} milliseconds", PeriodicDelay);

            while (stoppingToken.IsCancellationRequested)
            {
                _logger.LogInformation("Waiting for Periodic Timer tick in {PeriodicDelayMilliseconds} milliseconds", PeriodicDelay);
