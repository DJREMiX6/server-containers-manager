...
{
    internal class ResumeContainerCommandHandler(
        ILogger<ResumeContainerCommandHandler> logger, 
        AppDbContext dbContext,
        UserManager<AppUser> userManager,
        DockerClient dockerClient,
        TimeProvider timeProvider) : ICommandHandler<ResumeContainerCommand, ResumeContainerCommandResult>
    {
        private readonly ILogger<ResumeContainerCommandHandler> _logger = logger;
        private readonly AppDbContext _dbContext = dbContext;
        private readonly UserManager<AppUser> _userManager = userManager;
        private readonly DockerClient _dockerClient = dockerClient;
        private readonly TimeProvider _timeProvider = timeProvider;

        public async Task<ErrorOr<ResumeContainerCommandResult>> HandleAsync(ResumeContainerCommand command, CancellationToken cancellationToken = default)
        {
            ...
            var containerResumeResult = container.Resume(actor, now);
            if (containerResumeResult.IsError)
                return containerResumeResult.Errors;

            await _dockerClient.Containers.UnpauseContainerAsync(command.ContainerId, cancellationToken);

            await _dbContext.SaveChangesAsync(cancellationToken);

            return new ResumeContainerCommandResult();
        }
    }
}
