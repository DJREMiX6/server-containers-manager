...
{
    internal class PauseContainerCommandHandler(
        ILogger<PauseContainerCommandHandler> logger,
        ...
        TimeProvider timeProvider) : IQueryHandler<PauseContainerCommand, PauseContainerCommandResult>
    {
        ...
        public async Task<ErrorOr<PauseContainerCommandResult>> HandleAsync(PauseContainerCommand command, CancellationToken cancellationToken = default)
        {
            ...
            var containerPauseResult = container.Pause(actor, now);
            if (containerPauseResult.IsError)
                return containerPauseResult.Errors;

            await _dockerClient.Containers.PauseContainerAsync(command.ContainerId, cancellationToken);

            await _dbContext.SaveChangesAsync(cancellationToken);

            return new PauseContainerCommandResult();
        }
    }
}
